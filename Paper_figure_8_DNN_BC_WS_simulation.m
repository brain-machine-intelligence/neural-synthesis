function Paper_figure_8_DNN_BC_WS_simulation(gen_size_param, layer_num_param, idx_num)
addpath('./NN','./util')
rep = 1;
%DL
if nargin < 2
...
kf = 1;
try
load(strcat('saved_bc_ws_basic_training', num2str(gen_size),'_',num2str(layer_num),'_', num2str(idx_num),'.mat'));
catch

end

delete(strcat('saved_bc_ws_basic_training', num2str(gen_size),'_',num2str(layer_num),'_', num2str(idx_num),'.mat'));
...
%     [~, idx] = sort(maxd,'descend');
    ...
    for it_num = 1:n_iter
        ...
        for class_num = 1:2
            new_xN{class_num} = zeros(nval,30);
            origin_xN{class_num} = zeros(nval,30);
        ...
        for class_num = 1:2
            ...
            for i=1:nval
                ...
                [best, x0, bestObj, fitplotN{class_num}(i,:)] = pca_dl_fa_ndim_layer_normalize_iter_DNN_BCWS_batch_no_nn(x0,i,layer+1, dim_c, save_fa_data,nn);
                data_x(i,:) = x0;
                minT = min(x0);
                table = x0 - minT;
                maxT = max(table);
