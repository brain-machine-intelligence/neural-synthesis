function Paper_figure_4_n_5_DNN_MNIST_simulation(gen_size_param, layer_num_param, idx_num)
addpath('./NN','./util')
rep = 1;
%DL
if nargin < 2
...
for xx = [layer_num]
    ...
    end  %% 각  클래스 별 eigen vector 계산 및 저장
    ...
    for it_num = 1:n_iter
        ...
        for class_num = 1:10
            ...
            random_noise = rand(size(trainN_x{class_num}(kk_nt(1:nval),:)))*0.2 - 0.1;  % random noise 추가된 데이터 생성
       ...
       %% ex1 vanilla neural net
        ...
        meanErr = [];
        for ii = 1:avr_num
            regenerateNN = backup_regen_nn; % 백업해놓은 기존 network로 초기화
