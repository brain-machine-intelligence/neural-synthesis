...
function [bestsolution, x, bestojb, fitplot] = pca_dl_fa_ndim_layer_normalize_iter(x0,name,layer, dim, save_fa_data)
% parameters [n N_iteration alpha betamin gamma]
%para=[100 250 0.8 0.8 1];
para=[200 350 0.5 0.2 1];
%help fa_ndim.m
...
[u,fval,NumEval,~, fit]=ffa_mincon(@cost,u0,Lb,Ub,para, x0,layer, save_fa_data);
...
for i=2 : layer
    switch nn.activation_function 
            case 'sigm'
                ...
                sol.a{i}   =sigm(sol.a{i-1}  * nn.W{i - 1}');
                sol.a0{i}  =sigm(sol.a0{i-1}  * nn.W{i - 1}');
            case 'tanh_opt'
               sol.a{i}   =tanh_opt(sol.a{i-1}  * nn.W{i - 1}');
               sol.a0{i}  =tanh_opt(sol.a0{i-1}  * nn.W{i - 1}');
    end
    sol.a{i} = [ones(m,1) sol.a{i}];
    sol.a0{i} = [ones(m,1) sol.a0{i}];
end
DLx0 = sol.a0{layer};
DLx = sol.a{layer};
...
function [nbest,fbest,NumEval, x, fit]...
           =ffa_mincon(fhandle,u0, Lb, Ub, para, x0, layer,save_fa_data)
...
for i=2 : layer
    switch nn.activation_function 
            case 'sigm'
                % Calculate the unit's outputs (including the bias term)
                out.a{i}   =sigm(out.a{i-1}  * nn.W{i - 1}');
                out.a0{i}  =sigm(out.a0{i-1}  * nn.W{i - 1}');
            case 'tanh_opt'
               out.a{i}   =tanh_opt(out.a{i-1}  * nn.W{i - 1}');
               out.a0{i}  =tanh_opt(out.a0{i-1}  * nn.W{i - 1}');
    end
    out.a{i} = [ones(m,1) out.a{i}];
    out.a0{i} = [ones(m,1) out.a0{i}];
end
DLx0 = out.a0{layer};
DLx = out.a{layer};
